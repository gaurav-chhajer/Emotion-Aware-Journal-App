...
import { Calendar, ChevronDown, ChevronUp, Edit2, LogOut, Plus, Save, Trash2, User, BarChart2, Smile, Frown, Meh, Angry, Heart, Loader } from 'lucide-react';

// --- Firebase Configuration ---
const firebaseConfig = {
  apiKey: "AIzaSyAz--jpuEV-EO0AolSVRK9_JjeCFeMSSKU",
  authDomain: "emotion-aware-journal-app.firebaseapp.com",
  projectId: "emotion-aware-journal-app",
  storageBucket: "emotion-aware-journal-app.firebasestorage.app",
  messagingSenderId: "1061073916777",
  appId: "1:1061073916777:web:db75a081408fd43dbfe3f8",
  measurementId: "G-9RXJZDFTHV"
};
    ...
    <div className="flex items-center justify-center min-h-screen bg-gray-50 font-sans">
      <div className="w-full max-w-md p-8 space-y-6 bg-white rounded-2xl shadow-lg">
        <div className="text-center">
          ...
          <p className="mt-2 text-gray-600">Your personal space to reflect and understand.</p>
        </div>
...
const JournalApp = () => {
  ...
  useEffect(() => {
    if (user) {
      ...
      const unsubscribe = onSnapshot(q, (querySnapshot) => {
        ...
        entriesData.sort((a, b) => b.timestamp?.toDate() - a.timestamp?.toDate());
  ...
  return (
    <div className="bg-gray-100 min-h-screen font-sans text-gray-800">
      <Navbar onToggleDashboard={() => setShowDashboard(!showDashboard)} showDashboard={showDashboard} />
          ...
          {showDashboard ? (
            ...
            <>
              <JournalEditor 
                ...
                isSaving={isSaving}
              />
              <JournalFeed entries={entries} onEdit={startEdit} onDelete={deleteEntry} />
            </>
          )}
        </div>
      </main>
...
const JournalEditor = ({ currentEntry, setCurrentEntry, onSave, isEditing, onCancelEdit, isSaving }) => {
  return (
    <div className="bg-white p-6 rounded-2xl shadow-lg mb-8">
      <h2 className="text-xl font-semibold mb-4">{isEditing ? 'Edit Your Entry' : 'How are you feeling today?'}</h2>
      <textarea
        value={currentEntry}
      ...
      <div className="flex justify-end items-center mt-4 space-x-3">
        {isEditing && (
           <button onClick={onCancelEdit} className="px-4 py-2 text-sm font-semibold text-gray-600 bg-gray-200 rounded-lg hover:bg-gray-300 transition" disabled={isSaving}>
             Cancel
           </button>
        )}
        <button onClick={onSave} className="flex items-center justify-center px-6 py-2 font-semibold text-white bg-blue-600 rounded-lg hover:bg-blue-700 transition disabled:bg-blue-400" disabled={isSaving}>
...
const JournalFeed = ({ entries, onEdit, onDelete }) => {
  return (
    <div className="space-y-6">
      ...
      {entries.length > 0 ? (
        entries.map((entry) => (
          <JournalEntryCard key={entry.id} entry={entry} onEdit={onEdit} onDelete={onDelete} />
        ))
      ) : (
        <div className="text-center py-10 bg-white rounded-2xl shadow-md">
            <p className="text-gray-500">You have no entries yet.</p>
            <p className="text-gray-400 text-sm mt-1">Write one above to get started!</p>
        </div>
      )}
    </div>
  );
...
const JournalEntryCard = ({ entry, onEdit, onDelete }) => {
  ...
  const formattedDate = entry.timestamp ? new Date(entry.timestamp.toDate()).toLocaleDateString('en-US', {
    ...
    <div className="bg-white p-6 rounded-2xl shadow-md transition-all duration-300 hover:shadow-lg">
      <div className="flex justify-between items-start">
        ...
        <div className="flex items-center space-x-2">
           <button onClick={() => onEdit(entry)} className="p-2 text-gray-500 hover:text-blue-600 hover:bg-gray-100 rounded-full transition"><Edit2 className="w-5 h-5" /></button>
           <button onClick={() => onDelete(entry.id)} className="p-2 text-gray-500 hover:text-red-600 hover:bg-gray-100 rounded-full transition"><Trash2 className="w-5 h-5" /></button>
           <button onClick={() => setIsExpanded(!isExpanded)} className="p-2 text-gray-500 hover:text-gray-800 hover:bg-gray-100 rounded-full transition">
          ...
          <div className="mt-4">
            ...
            <div className="flex flex-wrap gap-2">
              {entry.keywords?.map((kw, i) => (
                <span key={i} className="px-3 py-1 text-xs font-medium text-blue-800 bg-blue-100 rounded-full">{kw}</span>
              ))}
            </div>
          </div>
        </div>
...
const Dashboard = ({ entries }) => {
  if (entries.length === 0) {
    return (
      <div className="text-center py-16 bg-white rounded-2xl shadow-lg">
        ...
        <p className="text-gray-600">Write some journal entries to see your emotional insights here!</p>
      </div>
    );
  }
  ...
  const COLORS = {
    'Joy': '#FFBB28', 'Sadness': '#0088FE', 'Anger': '#FF8042', 'Love': '#FF4560', 'Neutral': '#A0AEC0'
  };
  ...
  const activityData = Object.keys(entriesByDay).sort().map(day => ({
  ...
  const radarData = Object.entries(keywordCounts)
    .sort(([,a],[,b]) => b-a)
    .slice(0, 6)
    .map(([subject, A]) => ({ subject, A, fullMark: Math.max(...Object.values(keywordCounts)) }));
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div className="bg-white p-6 rounded-2xl shadow-lg">
          ...
          <ResponsiveContainer width="100%" height={300}>
            <PieChart>
              <Pie data={emotionData} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={100} label>
      ...
      <div className="bg-white p-6 rounded-2xl shadow-lg">
          ...
          <ResponsiveContainer width="100%" height={300}>
            <RadarChart cx="50%" cy="50%" outerRadius="80%" data={radarData}>
              ...
              <PolarRadiusAxis />
              <Radar name="Keywords" dataKey="A" stroke="#8884d8" fill="#8884d8" fillOpacity={0.6} />
              <Tooltip />
            </RadarChart>
          </ResponsiveContainer>
        </div>
